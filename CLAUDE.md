...
- `GET /api/radio/soundboard` · `POST /api/radio/transmit` — transmit console (R11): staged soundboard clips + available TTS engines, and operator-clicked TX (`clip` | TTS `text`+`engine`+`rate`, `settle_ms`); keys via `keyed_tx`, logs an `is_tx` row from the clean source. 409 = a transmit already in flight; 502/503 = rig refusal/unreachable; 500 = render/playback failure
