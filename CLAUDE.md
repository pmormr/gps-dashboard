...
- `radio_transmissions(id, started_utc, ended_utc, duration_s, freq_hz, mode, dcd_main, peak_dbfs, rms_dbfs, audio_path, lat, lon, waveform)` — one row per VOX-gate opening; the WAV lives under `/mnt/nvme/data/radio-audio/` and the retention pruner NULLs `audio_path` but keeps the row. `freq_hz`/`mode` are a rigctld snapshot of the **active main band** while the audio is SP1's A+B mix — `dcd_main` marks that tag's confidence; `lat`/`lon` snap from the latest raw fix (NULL when stale). `waveform` is a fixed-N JSON int array (0..255 bar heights, absolute dBFS window) derived at record time from the audio's sub-block peaks (`radio/waveform.py`) — drawn as the log/player waveform, and survives the audio prune.
