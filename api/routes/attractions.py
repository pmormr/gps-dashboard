...
* ``GET /api/attractions`` — bbox/kind/park/name-filtered POI list. Rows without
  a coordinate never match a bbox (their lat/lon is NULL) but appear in
  unfiltered or park/name-filtered reads.
* ``GET /api/attractions/<id>`` — one POI with parsed ``details``.
* ``GET /api/attractions/events`` — occurrences (event × date × time window) in
  a calendar-date window, grouped per event. Dates are park-local
  ``YYYY-MM-DD`` strings as published — not the ms-UTC axis.
* ``GET /api/attractions/events/<id>`` — one event with parsed ``details`` and
...
_ATTRACTION_COLUMNS = (
    'id, source, source_kind, source_id, park_code, name, lat, lon, summary, synced_at'
)
_EVENT_COLUMNS = (
...
@attractions_bp.get('/api/attractions')
def list_attractions():
    ...
    sql = f'SELECT {_ATTRACTION_COLUMNS} FROM attractions'
    if where:
        sql += ' WHERE ' + ' AND '.join(where)
    sql += ' ORDER BY name ASC LIMIT ?'
    conn = get_connection()
    rows = [dict(r) for r in conn.execute(sql, [*params, limit]).fetchall()]
    return jsonify({'attractions': rows, 'count': len(rows), 'truncated': len(rows) == limit})


@attractions_bp.get('/api/attractions/<int:attraction_id>')
def get_attraction(attraction_id: int):
    """One POI with its full parsed ``details`` (tour stops, hours, amenities…)."""
    conn = get_connection()
    row = conn.execute(
        f'SELECT {_ATTRACTION_COLUMNS}, details FROM attractions WHERE id = ?',
        (attraction_id,),
    ).fetchone()
    if row is None:
...
@attractions_bp.get('/api/attractions/events')
def list_events():
    ...
    sql = (
        ...
        'FROM attraction_event_dates d JOIN attraction_events e ON e.id = d.event_id'
    )
    if where:
        sql += ' WHERE ' + ' AND '.join(where)
...
@attractions_bp.get('/api/attractions/events/<int:event_id>')
def get_event(event_id: int):
    """One event with parsed ``details`` and its full occurrence list."""
    conn = get_connection()
    row = conn.execute(
        f'SELECT {_EVENT_COLUMNS}, details FROM attraction_events WHERE id = ?',
        (event_id,),
    ).fetchone()
    if row is None:
    ...
    record['dates'] = [
        ...
        for r in conn.execute(
            'SELECT date, time_start, time_end FROM attraction_event_dates '
