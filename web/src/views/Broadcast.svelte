<script lang="ts">
  ...
  // grouped by slot. Config is static, so load once (no poll); the live status
  // wall (two-sides dots + snapshots) is a separate Phase 2 surface.
  let data = $state<BroadcastFeeds | null>(null)
  let loadError = $state('')
  let copiedKey = $state<string | null>(null)
  let copyTimer: number | undefined

  let groups = $derived(data ? groupBySlot(data.feeds) : [])
...
<header class="page-head">
  ...
  <p class="muted">
    {#if loadError}<span class="err-text">{loadError}</span
      >{:else}Event-day feed config — grab-and-go send + OBS strings{/if}
  </p>
</header>
...
<style>
  ...
  /* Cap width on desktop — long URLs want horizontal room, not narrow columns. */
